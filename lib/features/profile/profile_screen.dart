...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // For technician, use AssignmentCubit; for others, use TicketCubit
    if (isTechnician) {
        ...
        builder: (context, state) {
          ...
          if (state is AssignmentSuccess) {
            ...
            resolvedTickets = state.assignments
                .where((a) => a.status == 'Completed' || a.status == 'Fixed' || a.status == 'Verified')
                .length;
            rate = totalTickets > 0
                ? (resolvedTickets / totalTickets * 100).round()
                : 0;
          }
          return _buildProfileContent(context, user, totalTickets, resolvedTickets, rate);
        },
      );
    }
  ...
  Widget _buildProfileContent(BuildContext context, UserModel user, int totalTickets, int resolvedTickets, int rate) {
    final bool isTechnician = user.role == 'technician';
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    isTechnician ? 'Technician' : (user.role == 'admin' ? 'Admin' : 'Citizen'),
                    style: const TextStyle(
                      fontSize: 13,
                ...
                Row(
                  ...
                  children: [
                    _buildStat('$totalTickets', isTechnician ? 'Assigned' : 'Tickets'),
                    Container(
                      width: 1,
                      height: 36,
                    ...
                    _buildStat('$resolvedTickets', isTechnician ? 'Completed' : 'Resolved'),
                    Container(
                      width: 1,
                      height: 36,
                      color: Colors.white.withOpacity(0.3),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      if (true) // was _authService.isEmailPasswordUser
                        _buildMenuItem(
